...
async function startVizServer(initialVizData, graph, projectRoot, port = 3333, shouldOpen = true, options) {
  ...
  const server = app.listen(availablePort, "127.0.0.1", () => {
    ...
    console.error(`
Cipher visualization running at ${url2}`);
...
async function connectToRepo(source, subdirectory, state) {
  try {
    ...
    if (isGitHub) {
      ...
      projectName = match[1];
      const reposDir = join3(tmpdir(), "cipher-repos");
...
async function restoreOriginal(dir, originalBranch) {
  ...
  try {
    execSync(`git checkout -q ${originalBranch}`, {
      // cipher-security-reviewed: branch validated above
      cwd: dir,
      stdio: "ignore"
    });
  } catch (error) {
...
async function stashChanges(dir) {
  try {
    ...
    if (status) {
      execSync('git stash push -q -m "cipher temporal analysis"', {
        cwd: dir,
        stdio: "ignore"
      });
      return true;
...
function ensureCipherDir(projectRoot) {
  const dir = join5(projectRoot, ".cipher");
  if (!existsSync3(dir)) {
...
function handleReleaseFiles(args, state) {
  const projectRoot = state.projectRoot;
  const claimsFile = join6(projectRoot, ".cipher", "claims.jsonl");
...
function handleGetActiveClaims(args, state) {
  const projectRoot = state.projectRoot;
  const claimsFile = join7(projectRoot, ".cipher", "claims.jsonl");
...
function ensureCipherDir2(projectRoot) {
  const dir = join8(projectRoot, ".cipher");
  if (!existsSync6(dir)) {
...
function handleRecordDecision(args, state) {
  const projectRoot = state.projectRoot;
  const dir = ensureCipherDir2(projectRoot);
...
function handleGetDecisions(args, state) {
  const projectRoot = state.projectRoot;
  const decisionsFile = join9(projectRoot, ".cipher", "decisions.jsonl");
...
function getToolsList() {
  return [
    {
      name: "connect_repo",
      description: "Connect Cipher to a codebase for analysis. Accepts a local directory path or a GitHub repository URL. If a GitHub URL is provided, the repo will be cloned automatically. This replaces the currently loaded project.",
      inputSchema: {
        type: "object",
        properties: {
          source: {
    ...
    },
    {
      ...
      description: "Retrieve auto-generated codebase documentation. Returns architecture overview, code conventions, dependency maps, and onboarding guides. Documentation must be generated first with `cipher docs` command.",
      inputSchema: {
        type: "object",
        properties: {
          doc_type: {
...
Returns ranked findings (Critical \u2192 Low) with attack scenarios and suggested fixes. Use --target for single-file scan.`,
    ...
    },
    {
      ...
      description: "Save a structured decision so future clients (or the same client in a future session) can see what was decided and why. Stored in .cipher/decisions.jsonl.",
      inputSchema: {
        type: "object",
        properties: {
          session_id: {
...
async function handleGetProjectDocs(docType, state) {
  const docsDir = join10(state.projectRoot, ".cipher");
  if (!existsSync8(docsDir)) {
    const errorMessage = `Project documentation has not been generated yet.

Run \`cipher docs ${state.projectRoot}\` to generate codebase documentation.
...
- onboarding: Guide for new developers`;
  ...
  if (!metadata) {
    return {
      content: [{ type: "text", text: "Documentation directory exists but metadata is missing. Please regenerate with `cipher docs`." }]
    };
  }
...
**Note:** The following documents are missing: ${missing.join(", ")}. Run \`cipher docs ${state.projectRoot} --update\` to generate them.`;
  }
  return {
...
async function handleGetTemporalGraph(state, commits, strategy) {
  ...
  try {
    ...
    const snapshots = [];
    const outputDir = join10(projectRoot, ".cipher", "temporal");
    ...
    if (snapshots.length === 0) {
      return {
        status: "no_snapshots",
        message: "No temporal snapshots found. Run `cipher temporal` to generate them.",
...
async function startMcpServer(state) {
  ...
  console.error("Cipher MCP server started");
