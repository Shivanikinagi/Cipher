...
function cacheDir(projectRoot) {
  return join2(projectRoot, ".cipher");
}
...
function findOutputJson(projectRoot) {
  return [
    join20(projectRoot, ".cipher", "cipher-output.json"),
    join20(projectRoot, "cipher-output.json")
  ];
}
...
function saveHealthHistory(projectRoot, report) {
  ...
  const historyFile = resolve16(resolvedRoot, ".cipher", "health-history.json");
...
function timestamp(version, date, fileCount, symbolCount) {
  return blockquote(`Auto-generated by Cipher ${version} on ${date} | ${fileCount.toLocaleString()} files, ${symbolCount.toLocaleString()} symbols`);
}
...
function generateOnboarding(graph, projectRoot, version) {
  ...
  output += header("Using Cipher with This Project", 2);
  output += generateCipherUsage(projectRoot);
  return output;
}
...
function generateHighImpactWarning(graph) {
  ...
  for (const { file, dependents } of topFiles) {
    output += `- ${code(file)} \u2014 ${dependents} dependent files (run \`cipher impact_analysis ${file}\`)
`;
  }
  output += "\n";
  return output;
}
function generateCipherUsage(projectRoot) {
  let output = "Use Cipher to explore this codebase:\n\n";
  ...
  output += "```bash\n";
  output += "cipher query . <symbol-name>\n";
  output += "```\n\n";
  output += "**Update documentation:**\n\n";
  output += "```bash\n";
  output += "cipher docs . --update\n";
  output += "```\n\n";
  return output;
}
...
function generateHistoricalTrend(projectRoot, currentReport) {
  const history = loadHealthHistory(projectRoot);
  if (history.length < 2) {
    return "No historical data available. Run `cipher health` regularly to track trends.\n\n";
  }
...
`;
    output += "---\n\n";
    output += "_This document was auto-generated by Cipher._\n";
    return output;
  }
  output += `Total symbols analyzed: **${formatNumber(report.totalSymbols ?? 0)}**
...
`;
  ...
  output += "\n---\n\n";
  output += "_This document was auto-generated by Cipher. It reflects the current state of the codebase and should be reviewed before taking action._\n";
  return output;
}
  ...
  try {
    for (const file of files) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes("cipher-security-reviewed")) continue;
        for (const pattern of PATTERNS) {
