...
`;
  output += `Cipher found 0 files it can analyze. Supported extensions:
`;
...
async function runTemporalAnalysis(projectDir, options) {
  ...
  try {
    const outputDir = options.output || join2(projectDir, ".cipher", "temporal");
...
// src/telemetry.ts
import os from "os";
var TELEMETRY_URL = "https://telemetry.cipher.dev/event";
function isOptedOut() {
  return process.env.CIPHER_NO_TELEMETRY === "1" || process.env.CIPHER_NO_TELEMETRY === "true" || process.env.DO_NOT_TRACK === "1";
}
...
<head>
  ...
  <title>Cipher \u2014 What If Simulation</title>
...
<body>
  <div class="whatif-header">
    <h1>cipher \u2014 What If Simulation</h1>
    ${opBadge}
  </div>
  ...
  <script>window.__cipherWhatIf = true;</script>
...
async function whatif(dir, options) {
  ...
  try {
    ...
    printResult(result);
    console.error(
      "\n\x1B[2m\u2192 Full report at app.cipher.dev \u2014 free to sign up\x1B[0m"
    );
...
function formatTable(result, elapsedMs) {
  ...
  const sep = "\u2500".repeat(62);
  lines.push("");
  lines.push(chalk2.bold("Cipher Security Scan"));
  lines.push("");
  const summaryParts = [
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      try {
        ...
        if (pkg.name === "cipher-cli") return pkg.version;
      } catch {
      }
      dir = dirname3(dir);
    }
...
function printUsage() {
  console.error(`Usage: cipher verify-change [options]
...
Input modes (exactly one required):
  ...
  cat file | cipher verify-change --file <path>   Piped stdin
...
async function computeDiff(commitA, commitB, projectRoot, options = {}) {
  ...
  try {
    ...
    if (status.length > 0) {
      git('stash push -m "cipher-diff-temp-stash"', projectRoot);
      stashed = true;
    }
  } catch {
...
function printUsage2() {
  console.error(`Usage: cipher diff <commit-a> <commit-b> [options]
...
function printHumanReadable2(result, options) {
  ...
  console.log("");
  console.log(c.bold(`Cipher diff: ${result.commit_a}..${result.commit_b}`));
  console.log(c.dim(line));
  console.log("");
...
var WORKFLOW_PROMPTS = {
  generic: `CIPHER WORKFLOW \u2014 Proven to reduce cost 19%, tokens 27%, and time 36% on complex multi-file tasks.
...
\u2192 Tests failing after 0 compiler errors =
  runtime dependency Cipher can't detect
\u2192 Fix those manually
...
2. DW Basic (Cipher available but no workflow)
   performs WORSE than no Cipher \u2014 you must follow
   this workflow for the benefit to materialize
3. Cipher finds static dependencies only.
   Runtime concerns (ORM registries, module caching,
   dynamic imports) are invisible to Cipher.
   Use the compiler/test suite for those.
4. AGENTS.md in .cipher/ contains project-specific
   context. Read it first before any analysis.`,
  claude: `CIPHER WORKFLOW (Claude/Cursor) \u2014 Proven to reduce cost 19%, tokens 27%, and time 36% on complex multi-file tasks.
...
\u2192 Read .cipher/AGENTS.md if present \u2014 it has project-specific context.
...
npm test / pytest / go test
\`\`\`
\u2192 Tests failing after 0 compiler errors = runtime dependency Cipher can't detect
\u2192 Fix those manually

## CRITICAL RULES
1. Run affected_files BEFORE writing any code (not after \u2014 this is the key insight)
2. DW Basic (Cipher available but no workflow) performs WORSE than no Cipher \u2014 you must follow this workflow for the benefit to materialize
3. Cipher finds static dependencies only. Runtime concerns (ORM registries, module caching, dynamic imports) are invisible to Cipher. Use the compiler/test suite for those.
4. AGENTS.md in .cipher/ contains project-specific context. Read it first before any analysis.`,
  cline: `CIPHER WORKFLOW (Cline) \u2014 Proven to reduce cost 19%, tokens 27%, and time 36% on complex multi-file tasks.

STEP 1 \u2014 ORIENT: connect_repo . then get_architecture_summary
  \u2192 Understand the module structure. Note which packages and directories exist.
  \u2192 Read .cipher/AGENTS.md if present for project-specific context.
...
- DW Basic (Cipher available but no workflow) performs WORSE than no Cipher \u2014 follow this workflow
- Cipher finds static dependencies only. Runtime concerns are invisible to Cipher. Use the compiler/test suite.
- AGENTS.md in .cipher/ contains project-specific context. Read it first.`,
  codex: `CIPHER WORKFLOW (Codex/CLI) \u2014 Proven to reduce cost 19%, tokens 27%, and time 36% on complex multi-file tasks.

1. connect_repo .
2. get_architecture_summary \u2192 understand module structure
3. Read .cipher/AGENTS.md if present \u2192 project-specific context
...
9. npm test / pytest / go test \u2192 runtime deps Cipher can't detect

CRITICAL RULES:
- Run affected_files BEFORE writing any code (not after \u2014 key insight)
- DW Basic (tools available, no workflow) performs WORSE than no Cipher
- Cipher finds static dependencies only. Runtime concerns invisible. Use compiler/tests.
- AGENTS.md in .cipher/ has project context. Read it first.`
};
...
function generateAgentsMd(graph, projectRoot, version) {
  ...
  lines.push("");
  lines.push(`> Auto-generated by Cipher v${version}. Re-run \`cipher parse\` to refresh.`);
  lines.push("");
  ...
  lines.push("## Cipher MCP Workflow");
  lines.push("");
  ...
  lines.push("```");
  lines.push("");
  lines.push("Full prompt: `cipher prompt` or `cipher prompt --tool claude`");
  lines.push("");
  return lines.join("\n");
}
function writeAgentsMd(graph, projectRoot, version) {
  const cipherDir = join5(projectRoot, ".cipher");
  const agentsPath = join5(cipherDir, "AGENTS.md");
  const content = generateAgentsMd(graph, projectRoot, version);
  try {
    if (!existsSync(cipherDir)) {
    ...
    console.error("\x1B[2mGenerated .cipher/AGENTS.md\x1B[0m");
  } catch (err) {
...
program.name("cipher").description("Code cross-reference graph builder for multi-language projects").version(packageJson.version);
program.command("parse").description("Parse a project and build dependency graph").argument("[directory]", "Project directory to parse (defaults to current directory or auto-detected project root)").option("-o, --output <path>", "Output JSON file path", "cipher-output.json").option("--pretty", "Pretty-print JSON output").option("--stats", "Print summary statistics").option("--exclude <patterns...>", 'Glob patterns to exclude (e.g., "**/*.test.*" "dist/**")').option("--verbose", "Show detailed parsing progress").action(async (directory, options) => {
  ...
  try {
    ...
    try {
      ...
      if (existsSync2(gitignorePath)) {
        ...
        const hasCipher = gitignoreContent.includes(".cipher") || gitignoreContent.includes("cipher-output.json");
        if (!hasCipher) {
          console.error(
            "\n\x1B[2mTip: Add these to .gitignore to keep your repo clean:\n  .cipher/\n  cipher-output.json\x1B[0m\n"
          );
        }
      }
    } catch {
...
program.command("query").description("Query impact analysis for a symbol").argument("<directory-or-symbol>", "Project directory or symbol name to query").argument("[symbol-name]", "Symbol name to query when a directory is provided").option("--json", "Output as JSON").option("--include-file-nodes", "Include file-level pseudo-nodes in JSON output").action(async (directoryOrSymbol, symbolArgument, options) => {
  ...
  try {
    ...
    const projectRoot = firstArgumentIsDirectory ? resolve7(directoryOrSymbol) : resolve7(".");
    const symbolName = firstArgumentIsDirectory ? symbolArgument : directoryOrSymbol;
    const cacheFile = resolve7("cipher-output.json");
    let graph;
    if (existsSync2(cacheFile)) {
      const json = JSON.parse(readFileSync5(cacheFile, "utf-8"));
      const cacheMatchesProject = !options.json || resolve7(json.projectRoot) === projectRoot;
    ...
    if (options.json) {
      ...
      const matches2 = findSymbols(graph, symbolName);
      ...
      if (matches2.length > 1) {
        console.log(JSON.stringify({
          error: "ambiguous",
          message: `Found ${matches2.length} symbols matching '${symbolName}'`,
          matches: matches2.map((match2) => ({
            id: match2.id,
            file: match2.filePath
          }))
        }, null, 2));
        process.exit(3);
      }
      ...
      const depths = /* @__PURE__ */ new Map([[match.id, 0]]);
    ...
    if (projectRootToConnect) {
      ...
      if (!noCache) {
        ...
        for (const jsonPath of candidates) {
          ...
          console.error(`[Cipher] Loading graph from ${jsonPath}...`);
          try {
            const ageMs = Date.now() - statSync(jsonPath).mtimeMs;
            const ageHours = ageMs / (1e3 * 60 * 60);
            if (ageHours > 24) {
              console.error(
                `[Cipher] Warning: cipher-output.json is ${Math.round(ageHours)} hours old. Run 'cipher parse .' to refresh.`
              );
            }
          } catch {
          }
          try {
            ...
            if (data.nodes && data.edges && data.projectRoot) {
              ...
              console.error(
                `[Cipher] Loaded graph from ${jsonPath} \u2014 ${graph.order} nodes, ${graph.size} edges. Run 'cipher parse .' to refresh.`
              );
            }
          } catch {
            graph = null;
          ...
          if (!graph) {
            ...
            if (parsedFiles) {
              ...
              console.error(
                `[Cipher] Loaded ${parsedFiles.length} files from cache. Run 'cipher parse .' to refresh.`
              );
            } else {
              console.error(
                `[Cipher] Could not load ${jsonPath} (invalid or empty).`
              );
            }
          }
          if (graph) break;
      ...
      if (!graph) {
        if (fromCache) {
          console.error(
            "[Cipher] Error: --from-cache specified but no cipher-output.json found. Run cipher parse . first."
          );
          process.exit(1);
        }
        console.error(`[Cipher] Parsing project: ${projectRootToConnect}`);
...
program.command("docs").description("Generate comprehensive codebase documentation").argument("[directory]", "Project directory to document (defaults to current directory or auto-detected project root)").option("-o, --output <path>", "Output directory (default: .cipher/ inside project)").option("--format <type>", "Output format: markdown | json", "markdown").option("--gitignore", "Add .cipher/ to .gitignore automatically").option("--no-gitignore", "Don't modify .gitignore").option("--include <docs>", "Comma-separated list of docs to generate (default: all)", "all").option("--update", "Regenerate existing docs").option("--only <docs>", "Used with --update, regenerate only specific docs").option("--verbose", "Show generation progress").option("--stats", "Show generation statistics at the end").option("--exclude <patterns...>", 'Glob patterns to exclude (e.g., "**/*.test.*" "dist/**")').action(async (directory, options) => {
  ...
  try {
    ...
    if (options.gitignore === void 0 && !existsSyncNode(outputDir)) {
      ...
      if (answer) {
        addToGitignore(projectRoot, ".cipher/");
      }
...
async function promptGitignore() {
  ...
  return new Promise((resolve8) => {
    rl.question("Add .cipher/ to .gitignore? [Y/n] ", (answer) => {
      rl.close();
...
program.command("prompt").description("Output Cipher MCP workflow prompt for AI agents").option("--tool <name>", "Agent tool: claude, cline, codex, generic (default: generic)", "generic").action((options) => {
