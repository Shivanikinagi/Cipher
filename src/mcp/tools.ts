...
async function handleGetProjectDocs(
  ...
  const docsDir = join(state.projectRoot!, '.cipher');
 ...
 */
function handleGetHealthScore(state: CipherState) {
  const graph = state.graph!;
...
async function handleGetTemporalGraph(
  ...
  try {
    ...
    const outputDir = join(projectRoot, ".cipher", "temporal");
...
function handleFindDeadCode(state: CipherState, confidence: string): any {
...
function handleSimulateChange(args: Record<string, any>, state: CipherState): any {
