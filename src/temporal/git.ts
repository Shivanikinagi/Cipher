...
export async function checkoutCommit(
  ...
  try {
    execSync(`git checkout -q ${hash}`, { cwd: dir, stdio: 'ignore' }); // cipher-security-reviewed: hash validated above
  } catch (error) {
...
export async function restoreOriginal(
  ...
  try {
    execSync(`git checkout -q ${originalBranch}`, { // cipher-security-reviewed: branch validated above
      cwd: dir,
      stdio: 'ignore',
    });
  } catch (error) {
