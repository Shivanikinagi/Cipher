 ...
 * Cipher never modifies or deletes any of your source files.
 * The only writes are: os.tmpdir() for cloned repos, and a local, git-ignored
 * parse cache at {projectRoot}/.cipher/cache.db used to skip re-parsing
...
export async function parseProject(
  ...
  // Restore unchanged files from {projectRoot}/.cipher/cache.db so only
...
/**
 * Candidate paths for a previously written `cipher parse` output file, in
 * priority order. `cipher mcp` probes these to load a pre-parsed graph on
 * startup instead of re-parsing from scratch.
 */
export function findOutputJson(projectRoot: string): string[] {
  return [
    join(projectRoot, '.cipher', 'cipher-output.json'),
    join(projectRoot, 'cipher-output.json'),
  ];
}

/**
 * Load a previously exported graph from a `cipher parse` JSON file and
 * reconstruct it as `ParsedFile[]` so it can flow through the normal
 * `buildGraph()` pipeline.
 *
 * `cipher parse` writes a serialized ProjectGraph — the on-disk shape is
...
export async function loadParsedFilesFromJson(
  ...
  try {
    ...
    // Shape 1: serialized ProjectGraph (what `cipher parse` actually writes).
