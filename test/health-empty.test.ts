...
describe('health score refuses to score when nothing was parsed', () => {
  ...
  beforeAll(() => {
    emptyDir = mkdtempSync(join(tmpdir(), 'cipher-health-empty-'));

    unparseableDir = mkdtempSync(join(tmpdir(), 'cipher-health-junk-'));
    writeFileSync(join(unparseableDir, 'a.txt'), 'hello world\n');
    writeFileSync(join(unparseableDir, 'b.csv'), 'a,b,c\n1,2,3\n');

    realDir = mkdtempSync(join(tmpdir(), 'cipher-health-real-'));
    writeFileSync(
