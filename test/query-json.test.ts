...
describe('cipher query --json', () => {
  let uniqueDir: string;
  let ambiguousDir: string;
  let emptyDir: string;

  beforeAll(() => {
    uniqueDir = mkdtempSync(join(tmpdir(), 'cipher-query-unique-'));
    writeFileSync(
    ...
    ambiguousDir = mkdtempSync(join(tmpdir(), 'cipher-query-ambiguous-'));
    writeFileSync(
    ...
    emptyDir = mkdtempSync(join(tmpdir(), 'cipher-query-empty-'));
  });
