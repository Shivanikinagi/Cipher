...
[![npm version](https://img.shields.io/npm/v/cipher-cli?color=00d4aa&label=npm)](https://www.npmjs.com/package/cipher-cli)
[![npm downloads](https://img.shields.io/npm/dm/cipher-cli?color=00d4aa&label=downloads%2Fmonth)](https://www.npmjs.com/package/cipher-cli)
[![GitHub stars](https://img.shields.io/github/stars/cipher/cipher?color=00d4aa&style=flat)](https://github.com/cipher/cipher/stargazers)
[![License](https://img.shields.io/badge/license-BUSL--1.1-00d4aa)](https://github.com/cipher/cipher/blob/main/LICENSE)
[![MCP Compatible](https://img.shields.io/badge/MCP-23%20tools-00d4aa)](https://github.com/cipher/cipher)

[![Languages](https://img.shields.io/badge/languages-17-0a1a14?style=flat)](https://github.com/cipher/cipher)
[![TypeScript](https://img.shields.io/badge/TypeScript-✓-3178c6?style=flat)](https://github.com/cipher/cipher)
[![Python](https://img.shields.io/badge/Python-✓-3776ab?style=flat)](https://github.com/cipher/cipher)
[![Go](https://img.shields.io/badge/Go-✓-00add8?style=flat)](https://github.com/cipher/cipher)
[![Rust](https://img.shields.io/badge/Rust-✓-ce422b?style=flat)](https://github.com/cipher/cipher)
[![Java](https://img.shields.io/badge/Java-✓-f89820?style=flat)](https://github.com/cipher/cipher)
[![PHP](https://img.shields.io/badge/PHP-✓-777bb4?style=flat)](https://github.com/cipher/cipher)
[![Ruby](https://img.shields.io/badge/Ruby-✓-cc342d?style=flat)](https://github.com/cipher/cipher)
[![Dart](https://img.shields.io/badge/Dart-✓-0175c2?style=flat)](https://github.com/cipher/cipher)
[![R](https://img.shields.io/badge/R-✓-276dc3?style=flat)](https://github.com/cipher/cipher)
[![+7 more](https://img.shields.io/badge/+7_more-C%2B%2B%20%7C%20C%23%20%7C%20Kotlin%20%7C%20Swift%20%7C%20Mojo%20%7C%20C%20%7C%20JS-555?style=flat)](https://github.com/cipher/cipher)
[![HTML/Angular](https://img.shields.io/badge/HTML%2FAngular-✓-e34c26?style=flat)](https://github.com/cipher/cipher)
...
[![Cloud](https://img.shields.io/badge/cloud-app.cipher.dev-00d4aa)](https://app.cipher.dev)
[![VS Code](https://img.shields.io/visual-studio-marketplace/v/cipher.cipher-vscode?label=VSCode&logo=visualstudiocode&color=00d4aa)](https://marketplace.visualstudio.com/items?itemName=cipher.cipher-vscode)

</div>

**Your AI doesn't know your architecture. Cipher does.**

## What makes Cipher different

<p align="center">
  <img src="./assets/deterministic_vs_rag_diagram.svg" alt="Cipher deterministic graph vs RAG probabilistic approach" width="680" />
</p>

Cipher builds a **DETERMINISTIC, NOT PROBABILISTIC** dependency graph of your codebase. This is not RAG. There are no embeddings, no similarity scores, no vector databases, no guesses. Cipher uses tree-sitter — the same parser powering GitHub's code intelligence — to extract exact symbol-level facts from every file: every function, every class, every interface, every import and export relationship, across 17 programming languages. When you ask "what breaks if I delete `encodeToken` in `auth/token.ts`?", Cipher does not search for similar-looking code and estimate an answer. It traverses the exact dependency graph and returns the precise list of 14 files that import that symbol, which import chains break, and what your health score drops by. This is compiler-level precision applied to AI-assisted development — not a language model's best guess about your code.

**Not a build graph either.** Tools like Nx, Turborepo, and Grapher track package-level dependencies for build caching. Cipher tracks symbol-level dependencies — every function, class, and import relationship — which is what makes What If simulation, graph-aware security scanning, and exact blast radius analysis possible.

## Contents

- [What makes Cipher different](#what-makes-cipher-different)
...
- [Cipher Action Token (DAT)](#cipher-action-token-dat)
- [Roadmap](#roadmap)

---

Cipher is the infrastructure layer between your AI coding assistant and your codebase. Before your AI touches a single file, Cipher has already mapped every connection, scored every risk, and simulated every change.

![Cipher CLI demo on honojs/hono](./assets/cipher-demo-cli.gif)

⭐ If Cipher saves you from a broken build, [star the repo](https://github.com/cipher/cipher) — it helps this project grow.

---

## Benchmark Results

Tested on [payloadcms/payload](https://github.com/payloadcms/payload) using **Claude Code (claude-opus-4-8)** — 645 files, 9,292 symbols, 80-file cascading refactor (adding a required parameter to a core error class used across the entire codebase). 3 modes tested: without Cipher, Cipher with no guidance, and Cipher with guided workflow prompt.
...
|------|----------|-----------|--------|------|-------------|
| Without Cipher | 16m 46s | 40 | 2,959,169 | $9.03 | 100% |
| Cipher (no guidance) | 11m 20s | 46 | 3,399,822 | $9.80 | 100% |
| Cipher + workflow | **10m 43s** | **26** | **2,165,356** | **$7.35** | 100% |

**Cipher + guided workflow vs no Cipher:**
- 36% faster
- 35% fewer API calls
- 27% fewer tokens
- 19% lower cost

> **Key finding:** Cipher without workflow guidance performed worse than no Cipher — agents had the tools but didn't use them unprompted, paying MCP overhead without getting any benefit. The `affected_files` command returned the complete 84-file blast radius in one call, but only when the agent was explicitly told to run it first. This is why we built `cipher prompt` — not as boilerplate, but because the benchmark showed agents need an explicit decision tree to use graph tools effectively.

[Full benchmark methodology and scripts →](https://github.com/cipher/cipher-benchmark)

> Agent: Claude Code (claude-opus-4-8) | Repo: payloadcms/payload @ commit 1545e87 | Benchmark scripts: [github.com/cipher/cipher-benchmark](https://github.com/cipher/cipher-benchmark)
...
![Cipher infrastructure layer](https://raw.githubusercontent.com/cipher/cipher/main/website/assets/cipher_infrastructure_layer.svg)

![Powered by DAT — open standard for AI agent action audit](https://raw.githubusercontent.com/cipher/cipher/main/website/assets/cipher_dat_governance_band.svg)

Cipher is the context and safety layer for AI-generated code.

Cipher sits between your AI and your codebase. It builds a complete dependency graph using tree-sitter — deterministic, not probabilistic — and serves it to your AI through 24 MCP tools.
...
- **Token-efficient** — Cipher serves pre-computed graph data. Your AI gets surgical answers, not file dumps. 40% fewer tool calls. 56% fewer file reads.
...
```bash
cipher whatif     # know what breaks before you change anything
cipher security   # catch vulnerabilities before AI ships them
cipher viz        # see your entire architecture instantly
```
...
> Numbers from real `cipher parse` runs on public repositories. Last validated: v1.8.2 (June 2026).
>
...
```bash
cipher whatif . --simulate delete --target src/utils/encode.ts
```
...
```bash
cipher whatif . --simulate delete --target src/utils/encode.ts
cipher whatif . --simulate move --target src/utils/encode.ts --destination src/core/encode.ts
cipher whatif . --simulate rename --target src/utils/encode.ts --destination src/utils/encoder.ts
cipher whatif . --simulate split --target src/services/auth.ts --symbols "validateToken,refreshToken"
cipher whatif . --simulate merge --target src/utils/helpers.ts --merge-target src/utils/formatters.ts
```
...
For multi-module Maven and Gradle projects, Cipher resolves imports across module boundaries — not just within a single module.

Example: simulating deletion of `Injector.java` in google/guice (a 13-module Java DI framework):

```
$ cipher whatif . --simulate delete --target core/src/com/google/inject/Injector.java
...
AI will confidently ship vulnerable code. Cipher stops it before production.

```bash
cipher security .                        # full repo scan
cipher security . --target src/auth.ts   # single file
cipher security . --format sarif         # GitHub Security tab integration
cipher security . --fail-on high         # CI gate — exit 1 if HIGH or above
cipher security . --class secrets         # specific check only
```
...
Graph-aware severity: a medium-severity finding reachable from an MCP tool or HTTP route is automatically elevated to critical. This is what no generic SAST tool can replicate — Cipher knows your architecture, so it knows what's actually reachable.

Available as MCP tool `security_scan` and via `cipher-cli/sdk`.
...
```bash
cipher verify-change --file src/auth.ts --content-from new-auth.ts
cipher verify-change --diff changes.patch
cipher verify-change --file src/auth.ts --content-from new-auth.ts --json
cat new-auth.ts | cipher verify-change --file src/auth.ts
```
...
```bash
cipher verify-change --diff pr.patch --fail-on-warnings --quiet
# exits 1 for medium risk, 2 for high risk
```

Available as MCP tool `verify_change` and CLI command `cipher verify-change`.
...
```bash
cipher diff main feature/auth-refactor
cipher diff HEAD~5 HEAD --verbose
cipher diff v1.5.0 v1.6.0 --json | jq
```

Example output:

    Cipher diff: v1.5.0..v1.6.0
    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    Symbols
...
![Cipher arc diagram visualization](./assets/cipher-demo-viz.gif)
...
![Cipher temporal graph on honojs/hono](./assets/cipher-temporal-hono.gif)
...
| Command | Description |
|---------|-------------|
| `cipher viz` | Interactive arc diagram in browser |
| `cipher whatif` | Simulate changes before touching code |
| `cipher verify-change` | Verify a proposed change is safe — broken imports, health delta, security |
| `cipher security` | Scan for vulnerabilities — graph-aware severity |
| `cipher health` | 0-100 architecture health score across 6 dimensions |
| `cipher dead-code` | Find unused symbols with confidence scoring |
| `cipher docs` | Generate 13 architecture documents |
| `cipher temporal` | Visualize architecture evolution over git history |
| `cipher parse` | Parse and export dependency graph as JSON |
| `cipher prompt` | Get the proven workflow prompt for your AI agent |
| `cipher diff` | Structural diff between two git commits — symbols, edges, health, security |
| `cipher mcp` | Start MCP server for AI coding assistants |

All commands auto-detect your project root. No path configuration needed.

### `cipher prompt` — proven workflow for AI agents

```bash
# Get the proven workflow prompt for your agent
cipher prompt                    # generic
cipher prompt --tool claude      # Claude Code optimized
cipher prompt --tool cline       # Cline optimized
cipher prompt --tool codex       # Codex optimized
```
...
Connect Cipher to any MCP-compatible AI tool. Your AI gets 23 tools it can call autonomously.
...
```bash
# Parse once (writes cipher-output.json)
cipher parse .
...
# Flags:
cipher mcp . --from-cache   # error if no cache found
cipher mcp . --no-cache     # force full re-parse
```
...
After running `cipher parse .`, Cipher generates `.cipher/AGENTS.md` — a project-specific context file containing module structure, key files, health summary, and MCP quick-start commands.

Claude Code reads `AGENTS.md` automatically when present. Add it to your `CLAUDE.md`:

```bash
# In your project root CLAUDE.md:
echo "## Cipher Context" >> CLAUDE.md
echo "Read .cipher/AGENTS.md for codebase architecture." >> CLAUDE.md
```
...
![Claude Desktop with Cipher MCP](./assets/claude.gif)
...
| `verify_change` | Safety report before applying code changes. Returns broken imports, circular deps, health delta, affected files. Also available as `cipher verify-change` CLI. |
...
#### `.cipher/` runtime state

The coordination tools (`claim_files`, `release_files`, `get_active_claims`, `record_decision`, `get_decisions`) write runtime state to `.cipher/claims.jsonl` and `.cipher/decisions.jsonl`. Add these to your project's `.gitignore`:

```
.cipher/claims.jsonl
.cipher/decisions.jsonl
```
...
Cipher detects connections between files written in different languages.

A TypeScript `fetch('/api/users')` call matched to a Python `@app.get('/api/users')` route definition — that's a cross-language edge. Delete the Python route and Cipher shows the TypeScript callers as broken.
...
Cipher exposes a stable public API for programmatic use and CI pipelines:

```bash
npm install cipher-cli
```
...
The SDK is the stable public API surface. All integrations should import from `cipher-cli/sdk` — never from internal paths.

---

## Why Cipher

| | Cipher | RAG-based tools | LLM scanning |
|--|---------|-----------------|--------------|
...
Cipher integrates into your CI/CD pipeline via the [cipher-action](https://github.com/cipher/cipher-action) GitHub Action.
...
Add to `.github/workflows/cipher.yml`:

```yaml
name: Cipher PR Impact
on:
  pull_request:
    branches: [main]
...
[GitHub Marketplace](https://github.com/marketplace/actions/cipher-pr-impact) — [cipher-action repo](https://github.com/cipher/cipher-action)

---

## Cloud dashboard

[app.cipher.dev](https://app.cipher.dev) — full dependency graph, health score, dead code report, and AI codebase chat in the browser. No local setup required.
...
Search **Cipher** in the VSCode Extensions panel or:

    ext install cipher.cipher-vscode

[View on Marketplace](https://marketplace.visualstudio.com/items?itemName=cipher.cipher-vscode)
...
Subscribe at [app.cipher.dev/subscribe](https://app.cipher.dev/subscribe).
...
- Structural diff between commits — `cipher diff`
- Public SDK — `cipher-cli/sdk`
- Cloud dashboard — app.cipher.dev
- PR Impact GitHub Action
- VSCode extension — v1.0.13, Mac + Windows, [marketplace](https://marketplace.visualstudio.com/items?itemName=cipher.cipher-vscode)
...
- Fast MCP startup — loads from cipher-output.json in <100ms regardless of project size
- `cipher prompt` — proven workflow prompt for AI agents (19% cheaper, 36% faster in benchmarks)
- Auto-generated `.cipher/AGENTS.md` project context after `cipher parse`
...
Cipher is read-only. It never writes to, modifies, or executes your code.
...
## Cipher Action Token (DAT)

Cipher is the reference implementation of the [Cipher Action Token (DAT)](https://github.com/cipher/dat-spec) — an open standard for cryptographically signing AI agent actions. DAT provides tamper-proof audit trails for every tool call, file change, and agent delegation.
...
Commercial licensing: hello@cipher.dev
